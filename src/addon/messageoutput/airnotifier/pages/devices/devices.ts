...
import {
  ...
  AddonMessageOutputAirnotifierDevice
} from "../../providers/airnotifier";
...
export class AddonMessageOutputAirnotifierDevicesPage implements OnDestroy {
  ...
  constructor(
    ...
    private airnotifierProivder: AddonMessageOutputAirnotifierProvider,
   ...
   */
  ionViewDidLoad(): void {
    console.log("getting fuckin devices");
    this.fetchDevices();
  }
  ...
  protected fetchDevices(): Promise<any> {
    return this.airnotifierProivder
      .getUserDevices()
      .then(devices => {
        ...
        devices.forEach(
          (device: AddonMessageOutputAirnotifierDeviceFormatted) => {
  ...
  enableDevice(
    ...
    this.airnotifierProivder
      .enableDevice(device.id, enable)
      .then(() => {
