...
export class CoreLoginHelperProvider {
  ...
  static FAQ_URL_IMAGE_HTML =
    '<img src="assets/img/login/faq_url.png" role="presentation">';
  static FAQ_QRCODE_IMAGE_HTML =
    '<img src="assets/img/login/faq_qrcode.png" role="presentation">';
  ...
  appLaunchedByURL(url: string): boolean {
    ...
    let siteData: CoreLoginSSOData, modal;
    ...
    this.initDelegate
      ...
      .then(data => {
        ...
        return this.handleSSOLoginAuthentication(
          siteData.siteUrl,
          siteData.token,
          siteData.privateToken,
      ...
      .then(() => {
        if (siteData.pageName) {
          ...
          this.appProvider
            .getRootNavController()
            .push(siteData.pageName, siteData.pageParams);
        } else {
  ...
  canRequestPasswordReset(siteUrl: string): Promise<any> {
    return this.requestPasswordReset(siteUrl)
      .then(() => {
        return true;
      })
      .catch(error => {
        return (
          error.available == 1 ||
          (typeof error.errorcode != "undefined" &&
  ...
  checkLogout(): void {
    ...
    if (
      !this.appProvider.isSSOAuthenticationOngoing() &&
      this.sitesProvider.isLoggedIn() &&
      this.sitesProvider.getCurrentSite().isLoggedOut() &&
      navCtrl.getActive().name == "CoreLoginReconnectPage"
    ) {
  ...
  confirmAndOpenBrowserForSSOLogin(
    ...
    if (showConfirmation) {
      promise = this.domUtils.showConfirm(
        this.translate.instant("core.login.logininsiterequired")
      );
    } else {
  ...
  forgottenPasswordClicked(
    ...
    this.canRequestPasswordReset(siteUrl)
      .then(canReset => {
        if (canReset) {
  ...
  getErrorMessages(
    ...
    if (requiredMsg) {
      errors.required = errors.requiredTrue = this.translate.instant(
        requiredMsg
      );
    }
    if (emailMsg) {
  ...
  getLogoutLabel(site?: CoreSite): string {
    ...
    return (
      "core.mainmenu." +
      (config && config.tool_mobile_forcelogout == "1"
        ? "logout"
        : "changesite")
    );
  }
  ...
  getOAuthIdFromParams(params: { [name: string]: any }): number {
    return params && typeof params.oauthsso != "undefined"
      ? Number(params.oauthsso)
      : undefined;
  }
  ...
  getSitePolicy(siteId?: string): Promise<string> {
    return this.sitesProvider.getSite(siteId).then(site => {
      ...
      return site.getConfig("sitepolicy", true).then(
        sitePolicy => {
  ...
  getValidIdentityProviders(siteConfig: any, disabledFeatures?: string): any[] {
    ...
    const validProviders = [],
      httpUrl = this.textUtils.concatenatePaths(
        siteConfig.wwwroot,
        "auth/oauth2/"
      ),
      httpsUrl = this.textUtils.concatenatePaths(
        siteConfig.httpswwwroot,
        "auth/oauth2/"
      );
    ...
    if (siteConfig.identityproviders && siteConfig.identityproviders.length) {
      siteConfig.identityproviders.forEach(provider => {
        ...
        if (
          provider.url &&
          (provider.url.indexOf(httpsUrl) != -1 ||
            provider.url.indexOf(httpUrl) != -1) &&
          !this.isFeatureDisabled(
            "NoDelegate_IdentityProvider_" + urlParams.id,
            siteConfig,
            disabledFeatures
          )
        ) {
    ...
    params = {
      siteUrl:
        url && typeof url == "string" && url !== ""
          ? url
          : Links.SYNCOLOGY_LMS_URL
    };

    if (setRoot) {
      return this.appProvider
        .getRootNavController()
        .setRoot(pageName, params, { animate: false });
    } else {
  ...
  hasSeveralFixedSites(): boolean {
    return (
      CoreConfigConstants.siteurl &&
      Array.isArray(CoreConfigConstants.siteurl) &&
  ...
  isSSOLoginNeeded(code: number): boolean {
    return (
      code == CoreConstants.LOGIN_SSO_CODE ||
      code == CoreConstants.LOGIN_SSO_INAPP_CODE
    );
  }
  ...
  loadPageInMainMenu(page: string, params: any): void {
    ...
    if (page == CoreLoginHelperProvider.OPEN_COURSE) {
      ...
      this.eventsProvider.trigger(CoreEventsProvider.LOAD_PAGE_MAIN_MENU, {
        redirectPage: page,
  ...
  protected openMainMenu(
    ...
    if (page == CoreLoginHelperProvider.OPEN_COURSE) {
      ...
      // Open the main menu.
      return navCtrl.setRoot(
        "CoreMainMenuPage",
        { redirectPage: page, redirectParams: params, urlToOpen: url },
        options
      );
    }
  }
  ...
  openBrowserForOAuthLogin(
    ...
    const loginUrl = this.prepareForSSOLogin(
      siteUrl,
      service,
      launchUrl,
      pageName,
  ...
  openChangePassword(siteUrl: string, error: string): void {
    this.domUtils
      .showAlert(this.translate.instant("core.notice"), error, undefined, 3000)
      .then(alert => {
  ...
  openInAppForEdit(
    ...
    if (!this.isOpenEditAlertShown && !this.waitingForBrowser) {
      ...
      currentSite
        .openInAppWithAutoLogin(siteUrl + path, undefined, alertMessage)
        .then(() => {
  ...
  passwordChangeForced(siteId: string): void {
    ...
    if (
      ...
      activePage.component.name == "CoreLoginChangePasswordPage"
    ) {
      return;
    }
  ...
  requestPasswordReset(
    ...
    return this.wsProvider.callAjax(
      ...
      .then(result => {
        ...
        if (this.isSSOLoginNeeded(result.code)) {
            ...
            promise
              .then(() => {
                ...
                this.openBrowserForSSOLogin(
                  ...
                  result.code,
                  result.service,
                  result.config && result.config.launchurl,
                  data.pageName,
                  data.params
                );
              })
          ...
          if (currentSite.isOAuth()) {
            ...
            const identityProviders = this.getValidIdentityProviders(
              result.config
            );
          ...
          if (
            ...
            if (
              activePage &&
              activePage.component &&
              activePage.component.name == "CoreLoginReconnectPage"
            ) {
              return;
            }
  ...
  shouldShowSSOConfirm(typeOfLogin: number): boolean {
    return (
      !this.isSSOEmbeddedBrowser(typeOfLogin) &&
      (!CoreConfigConstants.skipssoconfirmation ||
        String(CoreConfigConstants.skipssoconfirmation) === "false")
    );
  }
  ...
  protected showWorkplaceNoticeModal(message: string): void {
    // const link = this.appProvider.getAppStoreUrl({
  ...
  protected showDownloadAppNoticeModal(message: string, link?: string): void {
    ...
    //     const alertMessageEl: HTMLElement = alert
    //       .pageRef()
    //       .nativeElement.querySelector(".alert-message");
   ...
   */
  protected showNotConfirmedModal(
    siteUrl: string,
    email?: string,
    username?: string,
    password?: string
    ...
    let message;
    if (email) {
      message = this.translate.instant("core.login.emailconfirmsent", {
        $a: email
      });
    } else {
    ...
    if (username && password) {
      ...
      promise = this.wsProvider
        .callAjax("core_auth_resend_confirmation_email", {}, preSets)
        .catch(error => {
          // If the WS responds with an invalid parameter error it means the WS is avaiable.
          return Promise.resolve(
            error && error.errorcode === "invalidparameter"
          );
        })
        .finally(() => {
          modal.dismiss();
    ...
    promise.then(canResend => {
      if (canResend) {
        ...
        this.domUtils
          .showConfirm(message, title, okText, cancelText)
          .then(() => {
            ...
            const preSets = { siteUrl };
            this.wsProvider
              .callAjax("core_auth_resend_confirmation_email", data, preSets)
              .then(response => {
                const message = this.translate.instant(
                  "core.login.emailconfirmsentsuccess"
                );
                this.domUtils.showAlert(
                  this.translate.instant("core.success"),
                  message
                );
              })
              .catch(error => {
  ...
  sitePolicyNotAgreed(siteId?: string): void {
    ...
    if (
      activePage &&
      activePage.component &&
      activePage.component.name == "CoreLoginSitePolicyPage"
    ) {
      return;
    }
  ...
  treatUserTokenError(
    ...
    if (error.errorcode == "forcepasswordchangenotice") {
      ...
      this.showMoodleAppNoticeModal(
        this.textUtils.getErrorMessageFromError(error)
      );
    } else if (error.errorcode == "connecttoworkplaceapp") {
      this.showWorkplaceNoticeModal(
        this.textUtils.getErrorMessageFromError(error)
      );
    } else {
      this.domUtils.showErrorModal(error);
  ...
  validateBrowserSSOLogin(url: string): Promise<CoreLoginSSOData> {
    ...
    return this.configProvider.get(CoreConstants.LOGIN_LAUNCH_DATA).then(
      (data): any => {
        ...
        if (signature == params[0]) {
          ...
          this.logger.debug(
            "Invalid signature in the URL request yours: " +
              params[0] +
              " mine: " +
              signature +
              " for passport " +
