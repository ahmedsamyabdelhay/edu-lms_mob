...
export class CoreSitesProvider {
  ...
  protected appName = Strings.APP_NAME;
  protected appNameArabic = Strings.APP_NAME_ARABIC;
  ...
  constructor(
    ...
    this.dbReady = appProvider
      .createTablesFromSchema(this.appTablesSchema)
      .catch(() => {
        // Ignore errors.
      });
  ...
  checkSite(
    ...
    if (!this.urlUtils.isHttpURL(siteUrl)) {
      ...
      return this.checkSiteWithProtocol(siteUrl, protocol).catch(error => {
        ...
        return this.checkSiteWithProtocol(siteUrl, protocol).catch(
          secondError => {
  ...
  checkSiteWithProtocol(
    ...
    return this.siteExists(siteUrl)
      .catch(error => {
        ...
        return this.siteExists(treatedUrl)
          .then(() => {
        ...
        return temporarySite
          ...
          .then(
            data => {
              ...
              if (
                data.coreSupported ||
                (data.code != CoreConstants.LOGIN_SSO_CODE &&
                  data.code != CoreConstants.LOGIN_SSO_INAPP_CODE)
              ) {
                // SSO using local_mobile not needed, try to get the site public config.
                return temporarySite.getPublicConfig().then(
                  (config): any => {
                    publicConfig = config;
                  ...
                  (error): any => {
                    ...
                    if (error.available === 1) {
                      ...
                      if (error.errorcode == "codingerror") {
                        ...
                        return this.utils
                          .checkRedirect(siteUrl)
                          .then(redirect => {
                            if (redirect) {
                              error.error = this.translate.instant(
    ...
    function rejectWithCriticalError(
      message: string,
      errorCode?: string
    ): Promise<never> {
  ...
  siteExists(siteUrl: string): Promise<void> {
    return this.http
      .post(siteUrl + "/login/token.php", {})
      .timeout(this.wsProvider.getRequestTimeout())
      .toPromise()
      .catch(() => {
        // Default error messages are kinda bad, return our own message.
        return Promise.reject({
          error: this.translate.instant("core.cannotconnecttrouble")
        });
      })
      .then((data: any) => {
        if (data === null) {
          // Cannot connect.
          return Promise.reject({
            error: this.translate.instant("core.cannotconnect", {
        ...
        if (
          data.errorcode &&
          (data.errorcode == "enablewsdescription" ||
            data.errorcode == "requirecorrectaccess")
        ) {
          return Promise.reject({
          ...
          data.error &&
          data.error == "Web services must be enabled in Advanced features."
        ) {
          return Promise.reject({
    ...
    return promise.then(
      (data: any): any => {
        if (typeof data == "undefined") {
          return Promise.reject(
            this.translate.instant("core.cannotconnecttrouble")
          );
        } else {
          if (typeof data.token != "undefined") {
            ...
            if (typeof data.error != "undefined") {
              ...
              return Promise.reject(
                this.translate.instant("core.login.invalidaccount")
              );
            }
          }
        }
  ...
  newSite(
    ...
    let candidateSite = this.sitesFactory.makeSite(
      undefined,
      siteUrl,
      token,
      undefined,
        ...
        if (result == this.VALID_VERSION) {
          ...
          return this.getSite(siteId)
            ...
            .then(() => {
              ...
              return this.getSiteConfig(candidateSite)
                .catch(error => {
   ...
   */
  protected treatInvalidAppVersion(
    result: number,
    siteUrl: string,
    siteId?: string
  ): Promise<any> {
    let errorCode, errorKey, translateParams;
    ...
    return promise.then(() => {
      return Promise.reject({
        error: this.translate
          .instant(errorKey, translateParams)
  ...
  protected validateWorkplaceVersion(info: any): number {
    ...
    if (typeof this.isWPApp == "undefined") {
      this.isWPApp =
        !!WP_PROVIDER &&
        WP_PROVIDER.name == "AddonBlockProgramsOverviewModule" &&
  ...
  protected validateSiteInfo(info: any): any {
    if (!info.firstname || !info.lastname) {
      const moodleLink = `<a core-link href="${info.siteurl}">${
        info.siteurl
      }</a>`;

      return {
        error: "core.requireduserdatamissing",
        params: { $a: moodleLink }
      };
    }
  ...
  checkRequiredMinimumVersion(config: any, siteId?: string): Promise<void> {
    if (config && config.tool_mobile_minimumversion) {
      const requiredVersion = this.convertVersionName(
          config.tool_mobile_minimumversion
        ),
      ...
      return site.checkIfLocalMobileInstalledAndNotUsed().then(
        ...
        () => {
          return site
            ...
            .then(config => {
              return this.checkRequiredMinimumVersion(config)
                .then(() => {
                  this.login(siteId);
  ...
  isLoggedIn(): boolean {
    return (
      typeof this.currentSite != "undefined" &&
      typeof this.currentSite.token != "undefined" &&
  ...
  async deleteSite(siteId: string): Promise<void> {
    ...
    try {
      await this.appDB.deleteRecords(CoreSitesProvider.SITES_TABLE, {
        id: siteId
      });
    } catch (err) {
  ...
  async getSite(siteId?: string): Promise<CoreSite> {
    ...
    if (!siteId) {
      ...
      const data = await this.appDB.getRecord(CoreSitesProvider.SITES_TABLE, {
        id: siteId
      });
  ...
  makeSiteFromSiteListEntry(entry: any): Promise<CoreSite> {
    ...
    site = this.sitesFactory.makeSite(
      entry.id,
      entry.siteUrl,
      entry.token,
      info,
    ...
    sites.forEach(site => {
      if (!ids || ids.indexOf(site.id) > -1) {
        ...
        const siteInfo = site.info
          ? this.textUtils.parseJSON(site.info)
          : site.info;
        const basicInfo: CoreSiteBasicInfo = {
          id: site.id,
          siteUrl: site.siteUrl,
          fullName: siteInfo && siteInfo.fullname,
          siteName: CoreConfigConstants.sitename
            ? CoreConfigConstants.sitename
  ...
  async getLoggedInSitesIds(): Promise<string[]> {
    ...
    const sites = await this.appDB.getRecords(CoreSitesProvider.SITES_TABLE, {
      loggedOut: 0
    });
  ...
  async logout(): Promise<void> {
    ...
    if (this.currentSite) {
      ...
      promises.push(
        this.appDB.deleteRecords(CoreSitesProvider.CURRENT_SITE_TABLE, {
          id: 1
        })
      );
    }
  ...
  async restoreSession(): Promise<any> {
    ...
    try {
      const currentSite = await this.appDB.getRecord(
        CoreSitesProvider.CURRENT_SITE_TABLE,
        { id: 1 }
      );
  ...
  async setSiteLoggedOut(siteId: string, loggedOut: boolean): Promise<any> {
    ...
    return this.appDB.updateRecords(CoreSitesProvider.SITES_TABLE, newValues, {
      id: siteId
    });
  }
  ...
  async updateSiteInfo(siteId: string): Promise<any> {
    ...
    try {
      ...
      if (versionCheck != this.VALID_VERSION) {
        ...
        return this.treatInvalidAppVersion(
          versionCheck,
          site.getURL(),
          site.getId()
        );
      ...
      try {
        await this.appDB.updateRecords(
          CoreSitesProvider.SITES_TABLE,
          newValues,
          { id: siteId }
        );
      } finally {
  ...
  async getSiteIdsFromUrl(
    ...
    try {
      const siteEntries = await this.appDB.getAllRecords(
        CoreSitesProvider.SITES_TABLE
      );
      const ids = [];
      const promises = [];
  ...
  migrateSiteSchemas(site: CoreSite): Promise<any> {
    ...
    const promise = site
      .getDb()
      .createTablesFromSchema(this.siteTablesSchemas)
      .then(() => {
   ...
   * @return Promise resolved when done.
   */
  protected applySiteSchemas(
    site: CoreSite,
    schemas: { [name: string]: CoreRegisteredSiteSchema }
  ): Promise<any> {
    const db = site.getDb();

    // Fetch installed versions of the schema.
    return db
      .getAllRecords(CoreSitesProvider.SCHEMA_VERSIONS_TABLE)
      .then(records => {
        const versions = {};
        ...
        for (const name in schemas) {
          ...
          if (
            ...
            (schema.siteId && site.getId() != schema.siteId)
          ) {
          ...
          this.logger.debug(
            `Migrating schema '${name}' of ${
              site.id
            } from version ${oldVersion} to ${schema.version}`
          );

          let promise: Promise<any> = Promise.resolve();
          if (schema.tables) {
            promise = promise.then(() =>
              db.createTablesFromSchema(schema.tables)
            );
          }
          if (schema.migrate) {
            promise = promise.then(() =>
              schema.migrate(db, oldVersion, site.id)
            );
          }

          // Set installed version.
          promise = promise.then(() =>
            db.insertRecord(CoreSitesProvider.SCHEMA_VERSIONS_TABLE, {
              name,
  ...
  isStoredRootURL(
    url: string,
    username?: string
  ): Promise<{ site: CoreSite; siteIds: string[] }> {
    ...
    return this.getSiteIdsFromUrl(url, true, username).then(siteIds => {
      ...
      if (siteIds.length > 0) {
        ...
        return this.getSite(siteIds[0]).then(site => {
          const siteUrl = this.textUtils.removeEndingSlash(
              this.urlUtils.removeProtocolAndWWW(site.getURL())
            ),
            treatedUrl = this.textUtils.removeEndingSlash(
