...
export class CoreDomUtilsProvider {
  ...
  protected INPUT_SUPPORT_KEYBOARD = [
    "date",
    "datetime",
    "datetime-local",
    "email",
  ...
  constructor(
    ...
    configProvider
      .get(CoreConstants.SETTINGS_DEBUG_DISPLAY, false)
      .then(debugDisplay => {
  ...
  confirmDownloadSize(
    ...
    const getAvailableBytes = new Promise(
      (resolve): void => {
          ...
          this.fileProvider
            ...
            .then(availableBytes => {
              if (this.platform.is("android")) {
                ...
                if (
                  availableBytes < CoreConstants.IOS_FREE_SPACE_THRESHOLD ||
    ...
    const getAvailableSpace = getAvailableBytes.then(
      (availableBytes: number) => {
        if (availableBytes === null) {
          ...
          if (
            ...
            size.size > availableBytes - CoreConstants.MINIMUM_FREE_SPACE
          ) {
            return Promise.reject(
              this.translate.instant("core.course.insufficientavailablespace", {
                size: readableSize
              })
            );
          }

          return this.translate.instant("core.course.availablespace", {
    ...
    return getAvailableSpace.then(availableSpace => {
      ...
      if (size.size < 0 || (size.size == 0 && !size.total)) {
        ...
        return this.showConfirm(
          wifiPrefix +
            this.translate.instant(unknownMessage, {
        ...
        return this.showConfirm(
          wifiPrefix +
            this.translate.instant("core.course.confirmpartialdownloadsize", {
              size: readableSize,
              availableSpace: availableSpace
            })
        );
  ...
  extractDownloadableFilesFromHtml(html: string): string[] {
    this.logger.error(
      "The function extractDownloadableFilesFromHtml has been moved to CoreFilepoolProvider." +
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      if (
        url &&
        this.urlUtils.isDownloadableUrl(url) &&
        urls.indexOf(url) == -1
      ) {
        urls.push(url);
      }
  ...
  getElementMeasure(
    ...
    if (usePadding || useMargin || useBorder) {
      ...
      if (usePadding) {
        surround +=
          this.getComputedStyleMeasure(computedStyle, "padding" + priorSide) +
          this.getComputedStyleMeasure(computedStyle, "padding" + afterSide);
      }
      if (useMargin) {
        surround +=
          this.getComputedStyleMeasure(computedStyle, "margin" + priorSide) +
  ...
  getConnectionWarningIconHtml(): string {
    return (
      '<div text-center><span class="core-icon-with-badge">' +
  ...
  getElementXY(
    ...
    selector?: string,
    positionParentClass?: string
  ): number[] {
    let element: HTMLElement = <HTMLElement>(
        (selector ? container.querySelector(selector) : container)
      ),
      offsetElement,
      positionTop = 0,
      positionLeft = 0;
  ...
  private getErrorTitle(message: string): any {
    if (
      message == this.translate.instant("core.networkerrormsg") ||
      message ==
        this.translate.instant("core.fileuploader.errormustbeonlinetoupload")
    ) {
      return this.sanitizer.bypassSecurityTrustHtml(
        this.getConnectionWarningIconHtml()
      );
    }
  ...
  getErrorMessage(error: any, needsTranslate?: boolean): string {
        ...
        if (error.backtrace) {
          extraInfo +=
            ...
            this.textUtils.replaceNewLines(
              this.textUtils.escapeHTML(error.backtrace),
              "<br>"
            );
        }
    ...
    let message = this.textUtils.decodeHTML(
      needsTranslate
        ? this.translate
            .instant(error)
  ...
  waitElementToExist(findFunction: Function): Promise<HTMLElement> {
    ...
    promiseInterval.promise = new Promise(
      (resolve, reject): void => {
    ...
    els.forEach(el => {
      ...
      if (
        ...
        (trigger.indexOf("hover") == -1 &&
          trigger.indexOf("focus") == -1 &&
          trigger.indexOf("click") == -1)
      ) {
        return;
      }
  ...
  isElementOutsideOfScreen(
    ...
    return (
      elementMidPoint > window.innerHeight || elementMidPoint < scrollTopPos
    );
  }
  ...
  isRichTextEditorEnabled(): Promise<boolean> {
    if (this.isRichTextEditorSupported()) {
      return this.configProvider
        .get(CoreConstants.SETTINGS_RICH_TEXT_EDITOR, true)
        .then(enabled => {
          return !!enabled;
        });
    }
  ...
  restoreSourcesInHtml(
    ...
    media.forEach((media: HTMLElement) => {
      let newSrc =
        paths[this.textUtils.decodeURIComponent(media.getAttribute("src"))];
      ...
      if (media.tagName == "VIDEO" && media.getAttribute("poster")) {
        newSrc =
          paths[
            this.textUtils.decodeURIComponent(media.getAttribute("poster"))
          ];
        if (typeof newSrc !== "undefined") {
    ...
    anchors.forEach((anchor: HTMLElement) => {
      const href = this.textUtils.decodeURIComponent(
  ...
  getScrollTop(content: Content): number {
    return (
      (content &&
        content._scrollContent &&
        content._scrollContent.nativeElement.scrollTop) ||
      0
    );
  }
  ...
  scrollToElementBySelector(
    ...
    const position = this.getElementXY(
      content.getScrollElement(),
      selector,
      scrollParentClass
    );
    if (!position) {
  ...
  scrollToInputError(content: Content, scrollParentClass?: string): boolean {
    ...
    return this.scrollToElementBySelector(
      content,
      ".core-input-error",
      scrollParentClass
    );
  ...
  async showAlertWithOptions(
    ...
    const alertId = <string>(
      Md5.hashAsciiStr((options.title || "") + "#" + (options.message || ""))
    );
        ...
        if (options.buttons) {
          ...
          const cancelButton = <AlertButton>options.buttons.find(button => {
            return (
              ...
              typeof button.role != "undefined" &&
              typeof button.handler != "undefined" &&
              button.role == "cancel"
            );
          });
  ...
  showDeleteConfirm(
    ...
    return this.showConfirm(
      this.translate.instant(translateMessage, translateArgs),
      undefined,
  ...
  showErrorModalDefault(
    ...
    return this.showErrorModal(
      typeof errorMessage == "string" ? error : defaultError,
      needsTranslate,
      autocloseTime
    );
  }
      ...
      (resolve, reject): any => {
        placeholder =
          typeof placeholder == "undefined" || placeholder == null
            ? this.translate.instant("core.login.password")
            : placeholder;
  ...
  showTextareaPrompt(
    ...
    const modal = this.modalCtrl.create("CoreViewerTextAreaPage", params, {
      cssClass: "core-modal-prompt"
    });
    ...
    return (
      ...
      (el.tagName.toLowerCase() == "textarea" ||
        (el.tagName.toLowerCase() == "input" &&
          this.INPUT_SUPPORT_KEYBOARD.indexOf(el.type) != -1))
    );
  }
  ...
  triggerFormCancelledEvent(formRef: ElementRef, siteId?: string): void {
    ...
    this.eventsProvider.trigger(
      CoreEventsProvider.FORM_ACTION,
      {
        action: "cancel",
